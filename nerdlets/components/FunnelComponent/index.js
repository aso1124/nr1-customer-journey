...
export default class FunnelComponent extends React.Component {
  ...
  _constructFunnelNrql(series) {
    ...
    const query = `FROM ${funnel.event} SELECT funnel(${
      funnel.measure
    } ${steps
      .map(step => `, WHERE ${step.nrqlWhere} as '${step.label}'`)
      .join(' ')}) as 'steps' WHERE ${series.nrqlWhere} ${since}`
    return query
  }
  ...
  _buildQueryMap() {
    ...
    series.forEach(
      s =>
        (this.queryMap[s.label] = randomstring.generate({
          length: 12,
          charset: 'alphabetic',
        }))
    )
  ...
  _getData() {
    ...
    return NerdGraphQuery.query({ query }).then(({ data }) => {
      ...
      const results = {
        ...
        colors: [].concat(...series.map(s => get_color_set())), //eslint-disable-line
        values: [],
      }
      //console.debug(data);
      series.forEach(s => {
        ...
        if (_steps) {
          if (results.values.length === 0) {
            _steps.forEach(step => {
              results.values.push([Object.values(step)[0]])
            })
          } else {
            _steps.forEach((step, i) => {
              results.values[i].push(Object.values(step)[0])
            })
          }
        }
      })
  ...
  render() {
    return <div className="funnel" ref={ref => (this._ref = ref)} />
  }
}
